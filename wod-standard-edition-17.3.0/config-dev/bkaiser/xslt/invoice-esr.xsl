...
<xsl:stylesheet version="1.0"
 ...
 <!--  xmlns:fox="http://xmlgraphics.apache.org/fop/extensions"
  ...
  extension-element-prefixes="func exslt jm">
   -->
  <xsl:import href="lib-esr2fo-2.0.xsl"/>
    
  
  <xsl:variable name="esr-minimal-border">3mm</xsl:variable>
  
  <xsl:template match="invoices">
    <fo:root xsl:use-attribute-sets="defaults">
  ...
  <xsl:template match="invoice">
    <fo:page-sequence master-reference="A4">
      <fo:flow flow-name="xsl-region-body">
        ...
        <fo:block margin-top="1em" margin-bottom="2em">Wir bitten dich höflich um Begleichung dieses Betrages innert 30 Tagen mittels beiliegendem Einzahlungsschein.</fo:block>
        <fo:block margin-bottom="1em">Mit rudersportlichen Grüssen</fo:block>
        <fo:block margin-bottom="1em">Seeclub Stäfa, Finanzen, Anne Weyden</fo:block>
  ...
  <xsl:template match="categoryList">
    <fo:table table-layout="fixed" inline-progression-dimension="100%">
      ...
      <fo:table-header>
        <fo:table-row font-weight="bold" background-color="lightgray">
  ...
  <xsl:attribute-set name="defaults">
