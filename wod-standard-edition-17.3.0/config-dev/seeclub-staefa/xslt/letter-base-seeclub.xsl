...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:bk="http://www.bkaiser.com/2017/ns/seeclub" version="1.0">
  ...
  <xsl:template name="generic-letter-helpers">
    ...
    <xsl:if test="$debug &gt; 0">
      <fo:block-container absolute-position="fixed" left="0mm" top="0mm" width="21cm" height="29.7cm">
        <fo:block font-size="0">
          <fo:instream-foreign-object>
            <svg xmlns="http://www.w3.org/2000/svg" width="21cm" height="29.7cm" viewBox="0 0 210 297">
      ...
      <xsl:if test="$address">
        <!--
        <xsl:call-template name="address-window"><xsl:with-param name="left" select="'12mm'"/></xsl:call-template>
        -->
    ...
    <fo:block-container absolute-position="fixed" width="100mm" height="{$address-window-height}" top="{$top}" left="{$left}">
      <fo:block font-size="0">
        <fo:instream-foreign-object content-width="100mm" content-height="{$address-window-height}">
          <svg xmlns="http://www.w3.org/2000/svg" width="100mm" height="{$address-window-height}" viewBox="0 0 100 45">
          ...
          <fo:table-body>
            <fo:table-row>
              <fo:table-cell column-number="1">
                <fo:block font-weight="bold">Seeclub Stäfa</fo:block>
                <fo:block>
                  <fo:inline>
                    <fo:basic-link>
                      <xsl:attribute name="external-destination">                        http://www.seeclub-staefa.ch                      </xsl:attribute>                      www.seeclub-staefa.ch                    
              ...
              <fo:table-cell column-number="2">
                ...
                <fo:block>Grundwiesstrasse 29</fo:block>
                <fo:block>8700 Küsnacht</fo:block>
              </fo:table-cell>
              <fo:table-cell column-number="3">
                <fo:block>Tel. 079 291 7854</fo:block>
                <fo:block>
                  <fo:inline>
                    <fo:basic-link>
                      <xsl:attribute name="external-destination">                        mailto:aweyden@bluewin.ch                      </xsl:attribute>                      aweyden@bluewin.ch                    
